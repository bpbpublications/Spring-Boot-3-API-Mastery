...
public class ProductService {
    ...
    public Mono<ProductPageResponse> findAll(int pageNumber, int size, List<String> productCode) {
        ...
        Mono<Long> count;
        if(CollectionUtils.isEmpty(productCode)) {
            productEntityFlux = productRepository.findAllBy(pageable);
            count = productRepository.count();
        }
        else {
            productEntityFlux = productRepository.findAllByCodeIn(pageable, productCode);
            count = productRepository.countByCodeIn(productCode);
        }
        ...
        return productEntityFlux
                ...
                .map(productMapper::toProductResponse)
                .zipWhen(productResponses -> count)
    ...
    private Mono<ProductEntity> insertNewProduct(ProductRequest productRequest) {
        ...
        return productRepository.save(productEntity)
                        ...
                        .onErrorResume(DuplicateKeyException.class, e ->
                                Mono.error(new ProductAlreadyExistsException(productEntity.code())));
    }
