    ...
    public Mono<ResponseEntity<ProductResponse>> findProductByCode(String productCode, ServerWebExchange exchange) {
        return ReactiveSecurityContextHolder.getContext()
                .doOnNext(securityContext -> {
                    final Jwt principal = (Jwt) securityContext.getAuthentication().getPrincipal();
                    log.info("Called findProductByCode with productCode {} from {}", productCode, principal.getClaimAsString("email"));
                })
                .flatMap(securityContext -> productService.findProductByCode(productCode))
                .map(ResponseEntity::ok)
                .doOnSuccess(productResponseResponseEntity -> log.info("Terminated findProductByCode"));
    }
